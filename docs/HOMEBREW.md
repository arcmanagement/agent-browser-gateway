...
```text
dist/agent-browser-gateway-0.3.7-macos-arm64.zip
dist/agent-browser-gateway-extension-0.3.7.zip
...
```bash
make dist VERSION=0.3.7 CASK_OUTPUT=Casks/agent-browser-gateway.rb
git add Casks/agent-browser-gateway.rb
git commit -m "Update Homebrew cask for v0.3.7"
git push
```
