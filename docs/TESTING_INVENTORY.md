...
in the `Extension tests` job on Linux. Branch protection requires both check runs (`Swift tests`
and `Extension tests`) on `main`. The branch-protection connection is tracked by #23;
