...
Current `v0.4.4` status: ABG is not indexed by WinGet yet, and no Windows release ZIP has been
...
```powershell
.\scripts\update-winget-manifest.ps1 -Version 0.4.4
```

The output path is:

```text
dist\winget\manifests\a\ArcManagement\AgentBrowserGateway\0.4.4
```

Validate on Windows when `winget` is available:

```powershell
winget validate dist\winget\manifests\a\ArcManagement\AgentBrowserGateway\0.4.4
```
