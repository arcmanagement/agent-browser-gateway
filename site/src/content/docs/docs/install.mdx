...
Install with WinGet after the current release's `microsoft/winget-pkgs` PR is merged:

```powershell
winget install --id ArcManagement.AgentBrowserGateway --source winget
```

Until WinGet has indexed the current release, download the Windows x64 setup ZIP from the
[download page](/#download), extract it, and run `AgentBrowserGatewaySetup.exe`. The non-GUI payload
ZIP also includes `Install-AgentBrowserGateway.ps1` for scripted install.
