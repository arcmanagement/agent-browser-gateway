...
## Browser-Owned Personal Data

Bookmarks and Reading List entries are separate from shared-tab page state. Enable the Bookmarks or
Reading List toggle in the extension popup before using:
...
`bookmarks` uses Chrome's `chrome.bookmarks` API and optional `bookmarks` permission. `reading-list`
uses `chrome.readingList`, documented by Chrome for Chrome 120+ with the optional `readingList`
permission. Browsers that do not expose `chrome.readingList` return an explicit unsupported error.
Audit logs store command metadata, counts, ids, and byte lengths, not full bookmark or Reading List
URLs.
