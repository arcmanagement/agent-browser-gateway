...
現在の `v0.4.1` public release status:

- macOS DMG: `/downloads/agent-browser-gateway-0.4.1-macos-arm64.dmg` で公開済み
- Chrome extension: Chrome Web Storeでversion `0.4.1` として公開済み
...
extensionは次のChrome Web Store listingからインストールします。

```text
https://chromewebstore.google.com/detail/agent-browser-gateway/ojgedfcgebjchckaagjkmlpgonpjggpi
```

release verificationでは、public listingのversionが現在のrepository releaseと一致することを確認します。
`v0.4.1` では、listingがversion `0.4.1` を表示することを確認します。
