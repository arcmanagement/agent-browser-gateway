...
ユーザーは Chrome 拡張アイコンをクリックして「このタブを共有」を**明示的に許可**したタブだけを、`abg` CLI 経由で参照・操作できる。許可されていないタブには触れない (エラーになる)。

## 基本フロー

1. `abg status` で Gateway が起動しているか確認 (running: true なら OK)
2. `abg tabs` で共有中タブの ID と URL を確認
3. 必要に応じて read / screenshot / console / click / fill / type / key / navigate / scroll / wait を呼ぶ
...
- **canvas ベースのアプリ (Google Sheets, Figma, Google Docs 等) の操作**:
  ...
  - 文字入力は `type` (Input.dispatchKeyEvent 経由)。事前に対象セルにフォーカスを当てる
...
- v0.2 以降、書き込み系操作 (click/fill/type/key/navigate/scroll) はデフォルトで**ユーザー承認 popup** が出る。承認 OFF にする場合は拡張ポップアップから設定
