...
# app-group claim for Developer ID binaries: keeps Sequoia's group-container TCC
# prompts away when the brew CLI talks to a Mac App Store gateway (or vice versa).
GROUP_ENTITLEMENTS="packaging/developer-id/AgentBrowserGateway.developer-id.entitlements"
DIST_DIR="$ROOT/dist"
...
GITHUB_RELEASE_BASE_URL="${GITHUB_RELEASE_BASE_URL:-https://github.com/$GITHUB_REPOSITORY/releases/download}"
...
cask "agent-browser-gateway" do
  ...
  url "$GITHUB_RELEASE_BASE_URL/v#{version}/agent-browser-gateway-#{version}-macos-arm64.zip"
