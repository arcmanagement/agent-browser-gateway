...
For regular Windows use, extract `agent-browser-gateway-0.3.8-windows-x64-setup.zip` and double-click:
...
This restores packages, runs tests, builds `dist\agent-browser-gateway-0.3.8-windows-x64.zip`,
builds `dist\agent-browser-gateway-0.3.8-windows-x64-setup.zip`, extracts the non-GUI payload,
...
1. Extract `agent-browser-gateway-0.3.8-windows-x64.zip`.
2. In PowerShell, run:
