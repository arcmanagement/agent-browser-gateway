...
3. The CLI Unix socket is created with `chmod 0700`.
4. There is no MCP/CLI tool that executes arbitrary user-supplied JavaScript in a permitted tab. Tools are curated, structured, and named.
5. Any outbound network connection from the Gateway or extension is explicitly disclosed in the README, with the exact endpoint and purpose.
6. The audit log records every read and every operation, with the originating agent identifier where available.
