...
- Publish DMG and ZIP files through GitHub Releases. Never commit release binaries under `site/` or
  another source-tree path.
...
7. Copy the artifact hashes into `docs/TEMPORARY_ZIP_INSTALL.md`. Confirm `Casks/agent-browser-gateway.rb` has the app zip hash generated by `make dist` and points to the versioned GitHub Release asset.
